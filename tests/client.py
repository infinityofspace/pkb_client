...
class TestClientAuth(unittest.TestCase):
    ...
    @responses.activate
    def test_create_glue_record(self):
        pkb_client = PKBClient("key", "secret")

        responses.post(
            url=urljoin(API_ENDPOINT, "domain/createGlue/example.com/ns1"),
            json={"status": "SUCCESS"},
            match=[
                matchers.json_params_matcher(
                    {
        ...
        success = pkb_client.create_glue_record(
    ...
    @responses.activate
    def test_create_glue_record_single_ip(self):
        pkb_client = PKBClient("key", "secret")
        ...
        success = pkb_client.create_glue_record("example.com", "ns1", ["192.0.2.1"])

        self.assertTrue(success)

    @responses.activate
    def test_create_glue_record_error(self):
        pkb_client = PKBClient("key", "secret")
        ...
        with self.assertRaises(PKBClientException) as context:
            pkb_client.create_glue_record("example.com", "ns1", ["invalid.ip.address"])
