...
class TestBindFileParsing(unittest.TestCase):
    ...
    def test_reading_bind_file(self):
        with self.subTest("With default TTL"):
            ...
            self.assertEqual(BindRecord("test.com.", 600, RecordClass.IN, DNSRecordType.A, "1.2.3.4"),
                             bind_file.records[0])
            self.assertEqual(BindRecord("sub.test.com.", 600, RecordClass.IN, DNSRecordType.A, "4.3.2.1"),
                             bind_file.records[1])
            self.assertEqual(BindRecord("test.com.", 600, RecordClass.IN, DNSRecordType.AAAA, "2001:db8::1",
                                        comment="This is a comment"),
                             bind_file.records[2])
            self.assertEqual(BindRecord("test.com.", 1234, RecordClass.IN, DNSRecordType.TXT, "pkb-client"),
                             bind_file.records[3])
            self.assertEqual(BindRecord("test.com.", 600, RecordClass.IN, DNSRecordType.MX, "mail.test.com.", prio=10),
                             bind_file.records[4])
        ...
        with self.subTest("Without default TTL"):
            ...
            self.assertEqual(BindRecord("sub.test.com.", 600, RecordClass.IN, DNSRecordType.A, "4.3.2.1"),
                             bind_file.records[1])
            self.assertEqual(BindRecord("test.com.", 700, RecordClass.IN, DNSRecordType.AAAA, "2001:db8::1",
                                        comment="This is a comment"),
                             bind_file.records[2])
            self.assertEqual(BindRecord("test.com.", 700, RecordClass.IN, DNSRecordType.TXT, "pkb-client"),
                             bind_file.records[3])
            self.assertEqual(BindRecord("test.com.", 600, RecordClass.IN, DNSRecordType.MX, "mail.test.com.", prio=10),
                             bind_file.records[4])
    ...
    def test_writing_bind_file(self):
        records = [
            ...
            BindRecord("sub.test.com.", 700, RecordClass.IN, DNSRecordType.A, "4.3.2.1"),
            BindRecord("test.com.", 600, RecordClass.IN, DNSRecordType.AAAA, "2001:db8::1"),
            BindRecord("test.com.", 600, RecordClass.IN, DNSRecordType.TXT, "pkb-client"),
            BindRecord("test.com.", 600, RecordClass.IN, DNSRecordType.MX, "mail.test.com.", prio=10)
        ]
        bind_file = BindFile("test.com.", 1234, records)

        file_content = ("$ORIGIN test.com.\n"
                        "$TTL 1234\n"
